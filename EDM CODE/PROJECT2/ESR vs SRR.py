...
scores = np.column_stack([ESR, -srr])
pareto_mask = identify_pareto(scores)

pareto_points = np.column_stack([ESR[pareto_mask], srr[pareto_mask]])
pareto_points = pareto_points[pareto_points[:, 0].argsort()]
best_quality_idx = np.argmin(ESR)
best_productivity_idx = np.argmax(srr)
balanced_idx = np.argmin(np.sqrt((ESR - ESR.min())**2 + (srr - srr.max())**2))


# =============== Plotting Pareto Front ===================
plt.figure(figsize=(10, 8))
plt.scatter(ESR, srr, color='gray', alpha=0.6, s=50, label="Experimental Points")
...
plt.xlabel("ESR (µm)", fontsize=14, fontweight="bold")
plt.ylabel("SRR (mm³/min)", fontsize=14, fontweight="bold")
plt.xticks(fontsize=12, fontweight="bold")
plt.yticks(fontsize=12, fontweight="bold")
plt.title("Pareto Frontier for EDM Process\nTrade-off between ESR and SRR"
...
summary = pd.DataFrame({
    ...
    "Best Productivity (Max SRR)": best_productivity.values,

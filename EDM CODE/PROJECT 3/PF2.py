...
data = {
    ...
    "Surface Waviness": [4.174, 3.775, 2.457, 3.778, 4.174, 3.775, 2.716, 3.080, 3.530, 3.868,
                         ...
                         3.574, 3.574],
    "Material Vaporization SWte": [5.918, 5.631, 4.388, 5.686, 5.918, 5.631, 4.953, 5.214, 5.434, 5.735,
                                   ...
                                   5.563, 5.563],
    "Kerf Distance": [1.378, 1.277, 0.943, 1.289, 1.400, 1.286, 1.004, 0.965, 1.227, 1.311,
...
pareto_points = pareto_frontier(SW, KD, maximizeX=False, maximizeY=False)
plt.figure(figsize=(10, 8))
plt.scatter(SW, KD, color='gray', alpha=0.6, s=50, label='Experimental Points', zorder=1)
...
min_kerf_idx = np.argmin(KD)
balanced_idx = np.argmin(np.sqrt((SW - SW.min())**2 + (KD - KD.min())**2))

key_points = [
    (SW[best_quality_idx], KD[best_quality_idx],
     "Best SW\n({:.2f} μm)".format(SW[best_quality_idx]),
     's', 'red', (40, 20)),

    (SW[min_kerf_idx], KD[min_kerf_idx],
     "Min Kerf Width\n({:.3f} mm)".format(KD[min_kerf_idx]),
     '^', 'green', (-60, -30)),

    (SW[balanced_idx], KD[balanced_idx],
     "Balanced\n(SW={:.2f}, Kerf={:.3f} mm)".format(SW[balanced_idx],
                                                    KD[balanced_idx]),
     'D', 'purple', (50, -40))
]
...
plt.xlim(SW.min() * 0.95, SW.max() * 1.05)
plt.ylim(KD.min() * 0.95, KD.max() * 1.05)
plt.tight_layout()
plt.show()

# ====================== Summary Table ======================
best_quality = df.iloc[best_quality_idx]   # Minimum SW
best_kerf = df.iloc[min_kerf_idx]          # Minimum Kerf
balanced = df.iloc[balanced_idx]           # Balanced (SW + Kerf)
...
summary_table = pd.DataFrame({
    ...
    'Best Quality (Min SW)': [
        ...
        best_quality['Angle'],
        best_quality['Surface Waviness'],
        best_quality['Kerf Distance']
    ],
    'Min Kerf (mm)': [
        best_kerf['Pulse_On'],
        best_kerf['Pulse_Off'],
        ...
        best_kerf['Current'],
        best_kerf['Angle'],
        best_kerf['Surface Waviness'],
        best_kerf['Kerf Distance']
    ],
    'Balanced': [
        balanced['Pulse_On'],
        balanced['Pulse_Off'],

...
data = {
    ...
    "Surface Waviness": [4.174, 3.775, 2.457, 3.778, 4.174, 3.775, 2.716, 3.080, 3.530, 3.868,
                         ...
                         3.574, 3.574],
    "MVR": [5.918, 5.631, 4.388, 5.686, 5.918, 5.631, 4.953, 5.214, 5.434, 5.735,
...
kerf = df['Kerf Distance'].values  # ← FIXED
MVR = df['MVR'].values
...
scores = np.column_stack([kerf, -MVR])
pareto_mask = identify_pareto(scores)
pareto_points = np.column_stack([kerf[pareto_mask], MVR[pareto_mask]])
...
plt.scatter(kerf, MVR, color='gray', alpha=0.6, s=50, label='Experimental Points', zorder=1)
...
balanced_idx = np.argmin(np.sqrt((kerf - kerf.min())**2 + (MVR - MVR.max())**2))

key_points = [
    (kerf[best_quality_idx], MVR[best_quality_idx], "Min Kerf", 's', 'red'),
    (kerf[best_productivity_idx], MVR[best_productivity_idx], "Max MVR", '^', 'green'),
    (kerf[balanced_idx], MVR[balanced_idx], "Balanced", 'D', 'purple')
]
...
plt.xlabel('Kerf Distance', fontsize=14, fontweight='bold')
plt.ylabel('MVR ', fontsize=14, fontweight='bold')
plt.title('Pareto Frontier for EDM Process\nTrade-off between MVR and KD',
...
summary_table = pd.DataFrame({
    ...
    'Max MVR': [
        ...
        best_productivity['Kerf Distance'], best_productivity['MVR']
    ],
    'Balanced': [
        balanced['Pulse_On'], balanced['Pulse_Off'], balanced['Servo_V'],
        balanced['Current'], balanced['Angle'],
        balanced['Kerf Distance'], balanced['MVR']
    ]
})
